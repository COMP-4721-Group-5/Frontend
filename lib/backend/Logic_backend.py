...
class QwirkeleController:
    ...
    def __init__(
        self, client_list: List[ClientConnection], request_queue: Queue[Request]
    ) -> None:
    ...
    def process_request(self):
        ...
        if curr_request.data.request_type == "discard":
            ...
            if valid_discard:
                ...
                for i in range(len(discard_tiles)):
                    for j in range(6):
                        if curr_request.connection.get_player()[j] == discard_tiles[i]:
        ...
        # if request is placing tiles:
            ...
            if valid_placement:
                ...
                self.__get_curr_turn_client().get_player().score += (
                    self.__gamerules.score_move(curr_request.data, self.__board)
                )
                ...
                for placement in curr_request.data:
                    for j in range(6):
                        if curr_request.connection.get_player()[j] == placement.tile:
                            self.__tile_bag.append(
                                curr_request.connection.get_player()[j]
                            )
                            curr_request.connection.get_player()[j] = None
    ...
    def __start_next_turn(self):
        ...
        for i in range(6):
            if self.__get_curr_turn_client().get_player()[i] is None:
                self.__get_curr_turn_client().get_player()[i] = self.__tile_bag.pop(
                    rand.randrange(len(self.__tile_bag))
                )
