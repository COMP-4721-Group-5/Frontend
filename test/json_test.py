...
def test_tile_json(color: TileColor, shape: TileShape):
    ...
    assert test_tile == json.loads(
        json.dumps(test_tile, cls=JsonableEncoder), cls=JsonableDecoder
    )


@pytest.mark.parametrize("color", [TileColor.GREEN, TileColor.BLUE, TileColor.VIOLET])
...
def test_placement_json(color: TileColor, shape: TileShape, x: int, y: int):
    ...
    assert test_placement == json.loads(
        json.dumps(test_placement, cls=JsonableEncoder), cls=JsonableDecoder
    )
...
def test_board_json():
    ...
    assert base_board == json.loads(
        json.dumps(base_board, cls=JsonableEncoder), cls=JsonableDecoder
    )
